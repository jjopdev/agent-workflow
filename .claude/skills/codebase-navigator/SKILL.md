...
> For the generic RLM navigation protocol, see `~/.claude/skills/rlm-codebase-navigation/SKILL.md`
...
If the project has a `codebase-navigator` in `.claude/skills/` with the Module Map and Dependency Graph already filled in, **use that instead of exploring**. This generic skill is the fallback when a project-specific one does not exist.
