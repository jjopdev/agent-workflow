...
### [ARCH] SKILL.md is the canonical source for all agent definitions — never edit agent files directly; re-run bootstrap to sync
### [ARCH] Scribe agent owns all writes to .claude/skills/workflow-knowledge/ — Orchestrator delegates, never writes directly
### [DX] Model selection in Claude Code is manual — COMPLEXITY signal is guidance for the human operator, not automated routing
...
### [ARCH] Scribe scope expanded to include .claude/skills/security-context/ — only security-context, not arbitrary skills
